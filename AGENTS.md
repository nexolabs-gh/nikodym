...
**Siguiente: B1b… = resto de `core`** (config → audit → results → base/mixins → registry/artifacts → steps → lineage → study; ~30 archivos acoplados) hasta el primer `Study` end-to-end con lineage + los 3 criterios del Hito 0. Forma de trabajo: **yo-solo + fan-out**, sin equipo persistente aún. Tras F0 validado: T2 diseño (scoring) → F1 código → release público v0.1.0.
