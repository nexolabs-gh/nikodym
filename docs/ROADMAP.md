...
|---|---|---|---|
| **H1 · Scorecard sencillo** | Capa A (1.17.0, declarada experimental hasta B): puerta guiada `nikodym.Scorecard` (entrada mínima —con la frontera OOT exigida, D-OBL-5— e inferencias declaradas, `run()`/`run(until=)`/`resume()` como corrida nueva completa, decisiones humanas con motivo, resumen por etapa y resumen final con los dos estados, `compare`, MLflow opt-in, IV por muestra y alerta de monotonía fuera de desarrollo como artefactos aparte, notebook mínimo). Capa B: esenciales/«Avanzado» en pantalla, Excel opcional por etapa, `export()`. Capa C: informe con página ejecutiva, tipografía y marca, ficha renderizada | [`design/_ENMIENDA-FLUJO-GUIADO-SCORECARD.md`](design/_ENMIENDA-FLUJO-GUIADO-SCORECARD.md) (aprobada el 2026-09-18; capa A por implementar) | 1.17.0 (A) · 1.18.0 (B + C) |
