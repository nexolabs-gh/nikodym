...
|---|---|---|---|
| **H1 · Scorecard sencillo** | Capa A (1.17.0) y capa B (1.18.0; `guided` estable desde B): puerta guiada `nikodym.Scorecard` (entrada mínima —con la frontera OOT exigida, D-OBL-5— e inferencias declaradas, `run()`/`run(until=)`/`resume()` como corrida nueva completa, decisiones humanas con motivo, resumen por etapa y resumen final con los dos estados, `compare`, MLflow opt-in, IV por muestra y alerta de monotonía fuera de desarrollo como artefactos aparte, notebook mínimo). Capa B: esenciales/«Avanzado» en pantalla, Excel opcional por etapa, `export()`, cortes por variable (§8-9 (a)). Capa C: informe con página ejecutiva, tipografía y marca, ficha renderizada | [`design/_ENMIENDA-FLUJO-GUIADO-SCORECARD.md`](design/_ENMIENDA-FLUJO-GUIADO-SCORECARD.md) (aprobada el 2026-09-18; capas A y B implementadas, C por implementar) | 1.17.0 (A) · 1.18.0 (B) · 1.19.0 (C) |
