...
|---|---|---|---|
| C1 | **Usable por el banco chico** | notebook mínimo ≤ 25 líneas por módulo; ≤ 6 esenciales por sección; Excel/CSV entra y Excel opcional sale; todo en español; primer resultado ≤ 30 s con el dataset del paquete; **prueba con un tercero**: una persona sin Python construye y documenta un scorecard en una tarde con la guía (nombre, fecha y tiempo en el HANDOFF) ; **tres datasets distintos** corridos de punta a punta por el writer, al menos uno EXTERNO y real, antes de declarar un módulo completo (criterio de Cami, 2026-09-22) | H1 y cada hito |
...
|---|---|---|---|
| **H1 · Scorecard sencillo** | Capa A (1.17.0) y capa B (1.18.0; `guided` estable desde B): puerta guiada `nikodym.Scorecard` (entrada mínima —con la frontera OOT exigida, D-OBL-5— e inferencias declaradas, `run()`/`run(until=)`/`resume()` como corrida nueva completa, decisiones humanas con motivo, resumen por etapa y resumen final con los dos estados, `compare`, MLflow opt-in, IV por muestra y alerta de monotonía fuera de desarrollo como artefactos aparte, notebook mínimo). Capa B: esenciales/«Avanzado» en pantalla, Excel opcional por etapa, `export()`, cortes por variable (§8-9 (a)). Capa C (1.19.0): informe con página ejecutiva «Resumen de la corrida» desde la misma fuente que `summary()` y la pantalla, tipografía y paleta del sitio en HTML/PDF/Word, y ficha renderizada con el motivo de cada decisión humana (D-GOB-17) | [`design/_ENMIENDA-FLUJO-GUIADO-SCORECARD.md`](design/_ENMIENDA-FLUJO-GUIADO-SCORECARD.md) (aprobada el 2026-09-18; capas A, B y C implementadas) | 1.17.0 (A) · 1.18.0 (B) · 1.19.0 (C) |
...
3. **B2.3 · `[ui]`, uploads y presets.** Como delta, `[ui]` compone
   ...
   evaluada después de `await file.read()`). ⚠️ Ese tope **no** evita la transferencia ni el archivo
   temporal: FastAPI parsea el multipart antes de invocar el handler, y la lectura por chunks que
   este nodo prometía tampoco lo habría evitado. Cerrarlo exige un middleware sobre el stream ASGI
   —que además cubriría los cinco POST de JSON, hoy sin tope— y está **sin decidir**. Upload guarda los bytes originales en `uploaded_<sha256><suffix>` sin parsear
