...
Enmienda contractual **aprobada**:
[`_ENMIENDA-H9-ENTORNO-REPRESENTATIVO.md`](_ENMIENDA-H9-ENTORNO-REPRESENTATIVO.md). Retira
...
> declara ningún hueco, así que **cualquier** valor con ese `type` sale contestado—. ⚠️ Hay además
> **3 falsos negativos**, así que «válido para el motor» tampoco equivale a «respondida»: el criterio
> son **los dos**, conjuntos. 🔴 **Dos premisas del plan salieron falsas y abarataron la enmienda**:
> el front **sí** puede leer qué forma eligió el usuario en `partition.strategy` —es unión
> discriminada y sus tags igualan los `id` de forma, con gate vigente— y **no hace falta que el
...
> D-GOB-11/12/13/14 IMPLEMENTADAS el 2026-09-07 —S3—, D-GOB-15/16 IMPLEMENTADAS el 2026-09-08
> —S4—: enmienda COMPLETA y abierto 1 CERRADO).**
...
> exige una columna de grado que un scorecard no trae (override en el catálogo, D-EJE). Once
> revisiones adversariales del rango el mismo día; catorce hallazgos sobre esta enmienda, todos
...
> EDA no usado también se poda, la remisión a la ficha emitida es condicional porque sin
> `run_dir` ni inventario no hay ficha, el copy de `fail_on_falta_dato` se acota a los avisos
> declarados gobernables (una columna obligatoria ausente detiene siempre), `one_sided` sólo
> gobierna el t-test de severidad y exposición, y el backtesting sin IFRS 9 es dependencia del
> DAG y no aviso. Cinco capas con gates y controles negativos; el
...
| **T8 — Readiness integral** | 30 | Contratos productivos, integración, escala, informe y distribución por flujos. | SDD-30/H9R aprobados; W0 cerrada/PASS; W1 NO PASS/bloqueada por recalibración |
