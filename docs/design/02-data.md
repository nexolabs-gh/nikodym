...
- **D-DATA-7 — Frontera explícita: `data` modela el panel TRANSVERSAL de scorecard; IFRS 9/forward traen su capa de datos LONGITUDINAL propia** (Hito 0, CT-3; ver [`_CONTRATOS-TRANSVERSALES.md`](_CONTRATOS-TRANSVERSALES.md)). El modelo de datos de este SDD es un **panel transversal**: unidad = observación (índice = identificador de observación, §6), **una** ventana de desempeño, partición fija `{dev,ho,oot}` + rol `ttd`, target **binario** bueno/malo. *Decisión:* IFRS 9/ECL (F4) y forward (F5) son **panel longitudinal** por construcción (unidad = cuenta × período, exposición/EAD por `t`, múltiples escenarios `k`, PD_marg(t)/LGD(t)/EAD(t); ESPEC §5.5/§5.6) y **NO** reutilizan SDD-02 tal cual: tendrán su **propia capa de datos longitudinal** (módulo/contrato dedicado), que **deriva su `data_hash` del MISMO mecanismo** (contenido lógico por bloques, D-DATA-2) para ser comparable en mecánica aunque distinto en forma. El `LineageBundle` admite **múltiples fuentes identificadas por nombre de dataset** (no se asume un único `data_hash` global cuando coexistan paneles). *Porqué se fija ahora (y no el diseño, que se difiere):* sin esta frontera escrita, alguien asumiría que `data` sirve para todo y forzaría un refactor caro de SDD-02 —o un `data_hash` inconsistente entre paneles que rompe la prueba de reproducibilidad SR 11-7— al llegar a F4/F5. *Se difiere (a F4/F5):* el **diseño** de esa capa longitudinal (contrato, particiones por añada-de-reporting, staging Stage 1/2/3, definición de default regulatorio). *Responsable del diseño:* SDD de IFRS 9/forward (T4/T5).
