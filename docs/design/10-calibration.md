...
- `target_pd=None`: **sin ancla por defecto**. El `0.05` que este SDD describía como «placeholder ilustrativo» se retiró del código en `60013ac`; el default es `None`. Solo se fija cuando `anchor_source ∈ {"business_input", "historical_default_rate", "external_regulatory"}`, y con `anchor_source="development_observed"` **la configuración lo rechaza** en vez de descartarlo en silencio (D-ANC-1, ver [`_ENMIENDA-ANCLA-DESCARTADA.md`](_ENMIENDA-ANCLA-DESCARTADA.md)).
