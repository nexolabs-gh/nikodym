...
**Cableado futuro en `core.study`.** El código actual de `src/nikodym/core/study.py` contiene los tres dicts `_DOMAIN_MODULES`, `_DOMAIN_CONFIG_CLASSES` y `_DEFAULT_DOMAIN_ORDER` hasta `"calibration"`. Al implementar SDD-11, se agregan sin tocar lógica del motor:
