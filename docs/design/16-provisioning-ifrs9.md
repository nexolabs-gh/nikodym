...
- **Convención de descuento:** `DF(t) = (1 + EIR)^{−τ(t)}` con `τ(t)` el tiempo en años derivado de `time_value` y de la unidad temporal que **la propia term-structure declara** en su columna `time_unit` (D-HOR-0, implementado): el motor convierte con `core.time_units.year_fraction` y publica el resultado en `time_value_years`, junto al `time_value` crudo, para que la conversión sea un paso aritmético auditable. Si la curva no declara unidad convertible —columna ausente o un literal como `"period"`, el default de fábrica de SDD-18/SDD-19— se presume años y se emite `DATO-INSTITUCIONAL-IFRS-7`, gobernable por `fail_on_falta_dato`. La convención exacta (EIR anual + fracción de año vs EIR por período) se fija en config (D-IFRS-9); sólo `annual_eir_year_fraction` usa `τ`, `period_eir` usa el índice de período.

**Interfaz abstracta a la term-structure lifetime.** SDD-16 lee el artefacto `(<source>, "term_structure")` (o `result.term_structure()`), un `DataFrame` tidy con las columnas compartidas por SDD-18 y SDD-19: `[row_id?, segment?, partition?, period, time_value, time_unit?, hazard, survival, pd_marginal, pd_cumulative, method, pd_source, scenario, warning_codes]`. SDD-16 **valida** ese contrato y opera de forma agnóstica al método (KM/Cox/AFT/discrete-hazard/cohort/duration/Aalen-Johansen). No importa `nikodym.survival` ni `nikodym.markov`; el acoplamiento es solo el shape tidy (CT-2). `forward` (SDD-20) publica el mismo shape ya multiescenario y PIT-etiquetado, poblando la columna `scenario`.
...
- **Horizonte `H_12m` no conmensurable con la curva:** **cubierto desde D-HOR-0** por `FALTA-DATO-IFRS-8`. ⚠️ **El criterio NO es «el horizonte alcanza el soporte»**, que es lo que esta ficha describía antes como «modo A»: programarlo demostró que ese gatillo es **falso positivo** sobre el caso más común que existe —una curva mensual de 12 períodos con `horizon_12m_periods=12`, la configuración de fábrica, tiene el horizonte cubriendo toda la curva **y es correcta**: es una curva lifetime de doce meses, donde que Stage 1 iguale a Stage 2 es la contabilidad esperada—. Como la marca es gobernable, además abortaba esa corrida. Lo que el modo A intentaba aproximar sin poder medirlo es el **modo B**, que D-HOR-0 volvió medible: con la unidad declarada, el período del horizonte cae en `time_value_years` y se **verifica** que dure ~1 año (tolerancia ancha: cubre 360 vs 365 días y redondeos de calendario, y sigue cazando el error de orden de magnitud). Medido: con el default `12` sobre una curva anual, el «ECL a 12 meses» cubría doce años y sobrestimaba el Stage 1 unas **7,5 veces**, en silencio y con la unidad correctamente declarada. El chequeo sólo corre con unidad convertible —sin ella `time_value_years` es una presunción del propio motor, y `DATO-INSTITUCIONAL-IFRS-7` ya cubre el caso—. El segundo disyunto, **`H_12m` por debajo del primer período** (Stage 1 provisiona **cero**), no necesita unidad y se resuelve sobre cotas enteras de un frame no vacío, nunca sobre un estadístico de una selección posiblemente vacía —que devolvería `NaN`, y con él `False`, dejando el predicado mudo—. La máscara de `pd_pit.marginal_to_horizon` sigue siendo **independiente** de la de la ECL; lo que las descuadra a la vez es que leen el mismo campo de config.
...
- **Audit trail (`log_decision`).** Registrar como mínimo:
  ...
  - `ifrs9_pd_horizon`: registra `horizon_12m_periods` y `max_lifetime_periods` —ambos de config—, `card.falta_dato` y, desde D-HOR-0, el **soporte observado** de la curva (`soporte_periodos.min`/`.max`), que es contra lo que se contrasta el horizonte. Sin él no había forma de reconstruir por qué el aviso salió, o por qué no salió;
  - `ifrs9_discount_time_unit` (D-HOR-0): registra las **unidades observadas** en la curva recibida y si se presumieron años. `ifrs9_ecl` registra la convención *configurada*; ésta registra lo que llegó, que es lo que decide el exponente del descuento;
...
nunca emitió.** Sólo **IFRS-4, IFRS-6, IFRS-7 e IFRS-8 existen como código emitido en runtime**
(`ead.py`, `engine.py`; de ahí a `warning_codes` por fila, a `card.falta_dato` vía
`is_declared_warning()` y a la prosa del informe por `_IFRS9_WARNING_LABELS`). **IFRS-1, IFRS-2,
IFRS-3 e IFRS-5 son requisitos de entrada documentados**: nombran la exigencia y su política de
fallo, no una etiqueta que viaje en el resultado. `git grep` de cualquiera de esos cuatro sobre
`src/` devuelve cero, y eso es lo correcto — pero una ficha que dice «se registra» sin decir dónde
invita a leerlos como emitidos. Un código que aparece en un SDD y no en `src/` es una promesa, no un
contrato.

⚠️ **Y al revés: este catálogo es la fuente de verdad de la NUMERACIÓN, no `src/`.** El espacio
`IFRS-N` es **compartido por las dos marcas**, así que un `git grep` sobre `src/` da por libres los
números de los requisitos documentados. Al implementar D-HOR-0 se estuvo a punto de reutilizar
`IFRS-1` —que es el factor sistémico `Z`/`rho`— por exactamente ese motivo, y los dos tests
bidireccionales de `test_public_copy.py` habrían quedado **verdes**, porque comparan strings y no
semántica. **Antes de asignar un código nuevo se lee esta lista, no el código.**
...
- **DATO-INSTITUCIONAL-IFRS-7 — Unidad temporal de la term-structure (D-HOR-0, emitido).** La curva declara en qué unidad está su `time_value` (columna `time_unit`) y el motor la convierte a años con `core.time_units.year_fraction` antes de usarla como exponente del descuento, publicando `time_value_years` junto al crudo. Si no la declara —columna ausente, o un literal no convertible como `"period"`, el default de fábrica de SDD-18/SDD-19— se presume años y se emite este aviso **por `row_id`**, porque `forward` concatena N fuentes y unas filas pueden declararla y otras no. Es `DATO-INSTITUCIONAL` porque la periodicidad de la curva sólo la sabe la institución: el motor se niega a inventarla y hace explícita la presunción. Es **gobernable**: con `fail_on_falta_dato=True` —el default— detiene la corrida, y declarar la unidad lo hace desaparecer. Medido: la misma cartera en los mismos instantes, declarada en meses en vez de años, perdía ~40-50 % de provisión en silencio.
- **FALTA-DATO-IFRS-8 — Horizonte 12m no conmensurable con la curva (D-HOR-0, emitido).** Verifica que el período del horizonte dure ~1 año contra `time_value_years` (sólo con unidad convertible), y que no caiga por debajo del primer período. **No** usa «el horizonte alcanza el soporte»: ese criterio dispara sobre la curva lifetime de doce meses, que es legítima. Es `FALTA-DATO` y no `DATO-INSTITUCIONAL` porque *declarar* el horizonte es de la institución pero *verificar que sea coherente con lo recibido* es del motor —misma regla que FWD-8—, y por eso no viaja bajo `IFRS-2`, que se queda como requisito documentado. Sale **en bloque** y no por fila: es un escalar de config contra la curva. La auditoría `ifrs9_pd_horizon` expone el soporte observado.
