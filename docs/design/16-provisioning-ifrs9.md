...
- **Horizonte `H_12m ≥ min(T_max, max_lifetime_periods)`:** ⚠️ **degradación silenciosa conocida y NO cubierta.** El umbral es el mínimo, no `T_max`: `_prepare_term_structure` trunca la curva por `max_lifetime_periods` **antes** de todo. Alcanzado ese punto, la máscara `period <= horizon_12m` de `ecl.py` queda toda verdadera y **la ECL a 12 meses iguala a la lifetime: un Stage 1 provisiona lo mismo que un Stage 2**, sin warning, sin excepción y sin aviso declarado. (Con `stage3_direct=True` —no es el default— el Stage 3 se recalcula aparte y no iguala.) La máscara de `pd_pit.marginal_to_horizon` es **independiente** de la de la ECL; lo que las descuadra a la vez es que leen el mismo campo de config, no que una cause la otra. El motor tampoco contrasta `horizon_12m_periods` contra la unidad de `time_value` — y esa unidad no está fijada en ninguna parte, lo que afecta además al descuento. Diseño y decisión previa en [`_ENMIENDA-IFRS9-HORIZONTE.md`](_ENMIENDA-IFRS9-HORIZONTE.md). Hasta que se implemente, esta línea describe el comportamiento real: antes prometía un warning y un código `DATO-INSTITUCIONAL-IFRS-2` que nunca existieron en `src/`.
