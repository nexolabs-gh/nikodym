...
- **Horizonte `H_12m` no conmensurable con el soporte:** **cubierto desde D-HOR-0** por `FALTA-DATO-IFRS-8`, que declara dos degradaciones que antes eran silenciosas. (a) `H_12m ≥ min(T_max, max_lifetime_periods)`: el umbral es el mínimo, no `T_max`, porque `_prepare_term_structure` trunca por `max_lifetime_periods` **antes** de todo; alcanzado ese punto la máscara `period <= horizon_12m` de `ecl.py` queda toda verdadera y **la ECL a 12 meses iguala a la lifetime: un Stage 1 provisiona lo mismo que un Stage 2**. (Con `stage3_direct=True` —no es el default— el Stage 3 se recalcula aparte y no iguala.) (b) `H_12m <` el primer período de la curva: la máscara no selecciona nada y **Stage 1 provisiona cero**. El predicado se evalúa sobre cotas enteras de un frame no vacío, nunca sobre un estadístico de una selección posiblemente vacía —que devolvería `NaN`, y con él `False`, dejándolo mudo—. Un truncado **deliberado** (`max_lifetime_periods` por debajo del soporte bruto) queda exento: avisar de lo que el usuario pidió es ruido que enseña a ignorar el aviso. La máscara de `pd_pit.marginal_to_horizon` sigue siendo **independiente** de la de la ECL; lo que las descuadra a la vez es que leen el mismo campo de config, no que una cause la otra.
...
- **FALTA-DATO-IFRS-8 — Horizonte 12m no conmensurable con la curva (D-HOR-0, emitido).** Cubre el caso borde de §8 en sus dos formas (el horizonte se come la curva; el horizonte cae bajo su primer período). Es `FALTA-DATO` y no `DATO-INSTITUCIONAL` porque *declarar* el horizonte es de la institución pero *verificar que sea coherente con lo recibido* es del motor —misma regla que FWD-8—, y por eso no viaja bajo `IFRS-2`, que se queda como requisito documentado. Sale **en bloque** y no por fila: es un escalar de config contra el soporte de la curva. La auditoría `ifrs9_pd_horizon` expone el soporte observado.
