...
- **Columna `partition` presente y sin ninguna fila `desarrollo`:** `SurvivalInputError` nombrando las etiquetas observadas (**D-SUR-13**, ver «Decisiones para revisión de Cami»). Hasta el 2026-08-02 ese caso ajustaba **sobre la población completa**, en silencio y contradiciendo la columna que el propio usuario declaró; medido sobre una muestra Dev/HO con el efecto en signo opuesto, el coeficiente de la covariable pasaba de **+1,92 a −0,02** en `discrete_hazard` y de **+1,30 a −0,02** en `cox_ph`. El mensaje `"No hay filas de Desarrollo para ajustar…"` que los dos motores ya tenían escrito era inalcanzable salvo con el frame entero vacío, o sea justo cuando su texto miente.
- **Columna `partition` ausente:** el ajuste va sobre el frame completo y **eso se mantiene** — es el contrato standalone descrito en §4 —, pero el alcance deja de deducirse: se publica en `fit_scope_` y en `card.metric_sections["fit_scope"]`. `n_fit_rows == n_rows` no distinguía «no había partición» de «toda la cartera era Desarrollo».
...
- **SUR-6 → ✅ RESUELTO (2026-07-24).** El shape final IFRS 9 dejó de estar abierto cuando SDD-16 se implementó: fija las columnas económicas de ECL/stage/EIR/LGD/EAD/escenario, y el contrato entre ambas capas se ejercita en `tests/unit/test_provisioning_step.py` (el artefacto `survival/term_structure` consumido con `term_structure_source="survival"`) y en `tests/unit/test_ifrs9_engine.py`. Sale del conteo de brechas. *(Corrección del 2026-07-25: la primera redacción citaba `test_forward_ifrs9_contract.py`, que prueba la frontera forward↔IFRS 9 y no menciona survival. El cierre era correcto; la prueba ofrecida, no.)*
...
- **D-SUR-13 — Qué hace el motor cuando `partition` existe y no trae Desarrollo (IMPLEMENTADO 2026-08-02, reversible).** `_fit_mask` tenía **dos** salidas mudas y no eran el mismo caso. (a) *Sin columna* `partition` el ajuste va sobre el libro completo: es el contrato standalone de §4 y **se mantiene**, pero el alcance pasa a publicarse en `fit_scope_` y en `card.metric_sections["fit_scope"]`, porque `n_fit_rows == n_rows` no distinguía «no había partición» de «toda la cartera era Desarrollo». (b) *Con columna y sin ninguna fila* `desarrollo` **se levanta `SurvivalInputError`**, con las etiquetas observadas en el mensaje. Se descartó a propósito la vía del aviso declarado: no es `DATO-INSTITUCIONAL` —la institución **sí** aportó el dato— ni `FALTA-DATO` —el motor no difirió capacidad alguna—, y forzarlo a la taxonomía la corrompería; además, como esa marca sería **gobernable** y `fail_on_falta_dato` viene en `True`, la opción «blanda» detendría la corrida igual, sólo que después de haber hecho el ajuste equivocado. **Alcance medido:** ningún config de fábrica lo alcanza —el `Partitioner` exige `dev_fraction > 0` y valida particiones no vacías—, así que la suite completa (4.818 tests) pasa y el preset F4 sigue llegando a `done`. Quien sí lo alcanza es el uso por código (`DiscreteTimeHazardModel`/`CoxPHSurvivalModel`/`AFTSurvivalModel` son exports públicos) y la inyección `nikodym.run(..., artifacts={("data","frame"): …})`, que es puerta general por código. Si Cami prefiere que avise en vez de detener, el cambio es una línea en `nikodym/survival/partition.py` — pero entonces hay que decidir dónde queda constancia de que el número salió de otra población.
