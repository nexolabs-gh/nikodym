...
def all_nikodym_checks() -> "Iterator[tuple[str, Callable[[BaseNikodymEstimator], None]]]":
    """Generador (nombre_check, callable) para integrarse con parametrize (un nodo de
    test por check, igual que sklearn.utils.estimator_checks.parametrize_with_checks).
    NO recibe el estimador: cada `callable` toma una instancia FRESCA en el cuerpo del
    test (patrón sklearn — una instancia por check, sin estado compartido)."""
...
# all_nikodym_checks() NO recibe estimador; el nombre del check da el id del nodo.
@pytest.mark.parametrize("check", [c for _, c in all_nikodym_checks()],
                         ids=[name for name, _ in all_nikodym_checks()])
def test_forecaster_nikodym_contract(check):
    check(ArimaForecaster())   # una instancia FRESCA por check (patrón sklearn)
...
Lo que SDD-24 fija es la **configuración de las herramientas de test** en `pyproject.toml` (cuyo wiring final en CI es de SDD-25; aquí el contrato de contenido):
...
Verificación cruzada del caso PE: `0.05 × 0.45 × 1000 = 22.5` ✅ (aritmética verificada a mano). Los valores de oro de scorecard/Vasicek/ECL se calcularon desde las fórmulas citadas (`Factor=20/ln(2)≈28.853900`; `PD_PIT(0)≈0.012953`, `PD_PIT(1)≈0.004052`, `PD_PIT(−1)≈0.035341`; `ECL≈42.857143`) y son `entrada→salida` aseverables, no solo fórmulas. Para Vasicek se asevera además la **orientación del signo** (Z>0 expansión → PD menor: `0.004052 < 0.012953 < 0.035341`) con esos casos numéricos, porque el signo `−√ρ·Z` es la trampa documentada de la spec (ESPEC §5.5). Los canónicos usan `pytest.approx` con `abs`/`rel` explícito para floats no-exactos (scorecard, Vasicek, ECL); igualdad exacta para los aritméticos (PE).
...
- **Familia propia que un autor olvidó pasar por `check_nikodym_estimator`:** un **meta-test** (`tests/contract/test_all_families_covered.py`) recorre todas las subclases de las 4 bases propias registradas y verifica que cada una aparece en `contract/`; si falta, falla (cobertura del contrato, no del código).
...
- **Golden values** capturan el determinismo del `SeedManager` y de los canónicos: si una refactor cambia un valor de oro, el diff lo evidencia y exige justificación (es el audit-trail del determinismo).
...
- **Frontera exacta SDD-24 ↔ SDD-25** (qué del `pyproject.toml [tool.pytest]`/`[tool.coverage]` lo *especifica* SDD-24 vs lo *cablea en CI* SDD-25). *Sugerencia:* SDD-24 fija el **contenido** (markers, `fail_under`, `filterwarnings`); SDD-25 fija jobs/matriz/orden. *Responsable:* DanIA + autor **SDD-25**.
