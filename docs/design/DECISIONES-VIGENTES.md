...
|---|---|---|
| D-SIM-1…12 | **Propuesta el 2026-09-18 (S16)** sobre seis decisiones interactivas de Cami del mismo día (`run()` de una línea con parar y seguir; público primario el modelador que sabe estadística y no Python, con resultados legibles por etapa en notebook o pantalla y Excel opcional; esenciales visibles y «Avanzado» plegado; MLflow opt-in; identificadores en inglés y lectura en español; la simplicidad se aplica a la librería completa y el plan se pavimenta hasta terminarla). Contrato transversal: tres puertas, entrada mínima, defaults o constantes, cinco cifras por módulo, plantilla §13 obligatoria, poda sin ruptura. **Aprobada por Cami el mismo día, interactivamente y con la recomendación de cada punto de §12**: tope de **6** esenciales por sección; notebook mínimo **≤ 25** líneas; la frase de la directriz en `AGENTS.md`; poda en un **2.0 con censo**; orden scorecard → IFRS 9 → PD+LGD → validación/monitoreo → escala maestra → ML → forward/stress → originación. **Tras la pasada 1 de Codex (mismo día), Cami decidió**: D-OBL-5 se respeta (la puerta guiada no infiere la frontera OOT: la exige; la estrategia sigue a lo declarado); D-SIM-1 gana la cláusula de **adelanto declarado** (una puerta puede salir antes sólo marcada «experimental»); la paridad entre puertas es de **config, `config_hash` y resultados**, con la procedencia declarada; `resume()` es una **corrida nueva completa** (no reutiliza artefactos; D-ART intacta). 🔴 Aprobar el contrato no programa nada: cada módulo entra con su enmienda | [`31-simplicidad-y-flujo-guiado.md`](31-simplicidad-y-flujo-guiado.md) |
| D-FLU-1…12 | **Propuesta el 2026-09-18 (S16)**, primera aplicación de SDD-31 al scorecard: puerta guiada `nikodym.Scorecard`, etapas con resumen, `run(until=)`/`resume()`, decisiones humanas con motivo al trail, resumen final con los dos estados, Excel opcional, IV por muestra y alerta de monotonía fuera de desarrollo (aditivas), esenciales por sección (≈35 de 409), MLflow opt-in, notebook mínimo; capas A/B/C. Absorbe la forma de ENTREGABLES-LEGIBLES y los hallazgos #4/#5/#8 de INTEGRACION-EXTERNA-1-16. **Aprobada por Cami el mismo día con la recomendación de cada uno de los siete puntos de §8**: `nikodym.Scorecard`; la escala maestra como módulo propio (H5); **1.17.0 con la capa A (declarada experimental hasta B) y 1.18.0 con B + C**; tabla de esenciales como borrador aprobado (rótulos contra la pantalla al implementar); el notebook mínimo como único ejemplo canónico; los nombres del Excel de §3.5. **8-2 superada el mismo día tras Codex**: la frontera OOT **se exige** (`oot_from=`/`oot_cohorts=`; sin eje, `partition="random"` explícito), respetando D-OBL-5. Correcciones de la pasada 1 de Codex: `resume()` = corrida nueva completa; los diagnósticos del banco son **artefactos aditivos aparte** (`("selection","iv_by_partition")`, `("binning","event_rate_by_partition")`, sin mover `selection_table` ni las tablas de binning); la ficha sólo con `purpose=`. Pasada 2 de Codex (siete high, ninguno contractual, absorbidos): sin residuos de reanudación incremental; umbral de 30 **filas por tramo** como constante del diagnóstico (sin relación con `min_bads_per_partition`); autor y motivo como claves aditivas del trail con `DecisionRecord` intacto (el motivo llega a la ficha en la capa C); paridad medida sobre una proyección computacional sin procedencia; mapeo exhaustivo path esencial → argumento (≈ 37 esenciales); `run_dir="nikodym-runs"` y `name="scorecard"` por defecto (§8-8, veto de Cami). Pasada 3 (cinco high, absorbidos): `keep` escribe `selection.force_include` y `model.force_include`; `id` opcional (`unique_keys` o `index_col`); `DataFrame` como snapshot referenciado por el config; `partition="random"` con las tres fracciones fijadas; resúmenes sólo con lo publicado por su etapa o antes. Revisión de diseño cerrada en el tope de tres pasadas. ✅ **Capa A implementada en S17 (2026-09-19/20)** y publicada en la 1.17.0 (puerta guiada declarada experimental hasta B). **§8-9 respondido por Cami el 2026-09-20: (a)**, la hoja `user_splits`/`user_splits_fixed` en la capa B. ✅ **Capa B implementada en S18 (2026-09-21)**, tres sub-capas: B1 la pantalla (esenciales abiertos y UN bloque «Avanzado» cerrado con la cifra de campos que difieren de fábrica; la división la enciende la marca de sección `ui_essentials_declared` y sólo en las doce del scorecard; poda de sub-modelos, uniones y listas atómicas; el bloque se abre solo ante un error del motor o un foco pedido; golden del front espejo del de Python), B2 el Excel opcional por etapa (`export_excel()`, `01 Datos y muestras.xlsx` … `10 Validación formal.xlsx` + `11 Decisiones.xlsx`, celda a celda con los exports del informe) y `export("corrida.zip")`, más Resultados sobre la misma fuente que `summary()` (`results.json` gana `summaries`), y B3 los cortes por variable con `bins`/`merge_bins`/`set_bins` (perillas del scorecard 409 → 413 por la única excepción aprobada). Correcciones medidas anotadas en §3.5 y §3.9 de la enmienda. ✅ **1.18.0 publicada el 2026-09-21** con B sola (decisión interactiva de Cami). ✅ **Capa C implementada en S19 (2026-09-21)**, tres sub-capas: C1 la página ejecutiva «Resumen de la corrida» tras la portada (capítulo `kind="summary"` que el builder arma desde el `Study` con los constructores de `summary()`; dice qué corrió antes del informe y qué queda por correr, porque `report` es un paso y el informe se escribe durante la corrida), C2 la tipografía del sitio (Roboto 400/700 incrustada en el CSS, Word con Roboto y la paleta de la marca; PDF verificado en CI) y C3 la ficha con motivo (D-GOB-17). Anotado en §3.3 y §3.12 de la enmienda. 🔴 La 1.19.0 (cierra H1) y la recaptura de la demo, con su OK propio | [`_ENMIENDA-FLUJO-GUIADO-SCORECARD.md`](_ENMIENDA-FLUJO-GUIADO-SCORECARD.md) |
| D-SC-17 · D-SC-18 | **Aprobada por Cami el 2026-09-22**, interactivamente y con la recomendación de los tres puntos de su §8. Nace del criterio de completado del scorecard que Cami fijó el 2026-09-22 y del tercer dataset —UCI German Credit, externo y sin columna de fecha— que **fallaba**: con `partition="random"` sobre un archivo sin columna datetime —el caso que SDD-31 §8 declara soportado— la corrida moría en `eda` con `EdaError` y con ella las nueve etapas siguientes. **D-SC-17**: el motor degrada —tabla `by_period` vacia, causa `sin_eje_temporal` declarada, `overall_rate` global con la regla de hoy (`NaN` sin elegibles), decisión `tasa_por_periodo`/`no_evaluable` en el trail— extendiendo a la tasa el mecanismo que D-SC-2 le dio a la estabilidad; la regla entra **sólo** con las cuatro condiciones de §2 y los cinco errores del motor quedan intactos (`date_col` ausente o no datetime, dos columnas datetime, `axis="cohort"` sin `cohort_col` o con una inexistente). **D-SC-18**: qué dice cada una de las dieciocho superficies censadas, incluida la opción `eda.default_rate.axis="period"` del catálogo, que pasa a **«disponible»** con el aviso en su `help` porque «exige otro campo» está definido como «hasta que lo declares el config no se construye» y eso deja de ser verdad (la opción «cohort» conserva la marca). Presupuesto de perillas **cero**; ningún `config_hash` se mueve; las cinco cifras no cambian. Tres pasadas de Codex sobre el documento, las tres con hallazgos reales y distintos, absorbidas; tope declarado en §8bis | [`_ENMIENDA-EDA-SIN-EJE-TEMPORAL.md`](_ENMIENDA-EDA-SIN-EJE-TEMPORAL.md) |
| D-SC-19 · D-SC-20 | **Aprobada por Cami el 2026-09-23** e **implementada el mismo día** (las cinco precisiones del código, en su §7), interactivamente y con la recomendación de sus dos decisiones; la dirección —«el análisis exploratorio nunca detiene la corrida»— la eligió él mismo tras juzgar que D-SC-17 era demasiado estrecha («no puede morir por un error tan chico»), y la medición le dio la razón: una cartera con dos columnas de fecha y partición aleatoria seguía muriendo en `eda` en 3,5 s. **Generaliza D-SC-17 y deroga su §2 y su §8.2.** **D-SC-19**: `EdaStep.execute` nunca levanta y siempre publica sus seis artefactos; tasa, estabilidad, perfiles y calidad fallan **por separado**; se atrapan todas las excepciones, pero las inesperadas se publican con su tipo; un constructor nuevo (`tasa_no_calculable`) nunca levanta; el preflight deja de predecir el corte —el requisito de cohorte ya no se declara y el recorrido exime a `eda`, conservando `column_role` para el selector—; la opción «por cohorte» del catálogo pasa a «disponible». Nada caído se publica como negativo: calidad vacía en vez de ceros y métricas omitidas. **D-SC-20**: `failed_analyses` en la card (aditivo), una **alerta** por análisis caído, y la página ejecutiva dice «parcial», no «sin fallos». Las piezas siguen levantando como API estable; ninguna corrida que hoy termina cambia un número; perillas: cero. Tres pasadas de Codex absorbidas | [`_ENMIENDA-EDA-NUNCA-DETIENE.md`](_ENMIENDA-EDA-NUNCA-DETIENE.md) |
| D-RAR-1 · D-RAR-2 | **Aprobada por Cami el 2026-09-22**, interactivamente y con la recomendación de las cinco decisiones de su §6; **implementada el 2026-09-23** (seis precisiones del código en su §8; German Credit termina `done`). Nace del tercer dataset del criterio de completado del scorecard: tras D-SC-17, UCI German Credit pasa `eda` y muere en «Tramos y WoE» porque el nivel `A48` de `proposito` queda con 5 buenos y 0 malos en desarrollo. Medido con OptBinning real: el default `cat_cutoff = 0.01` es el valor que deja ese nivel **solo** en su grupo de raras —es el único bajo el 1 %—, y un corte que aísla exactamente un nivel crea un bin unitario, que es el candidato natural a tener una clase en cero. **D-RAR-1**: el **motor** —no la puerta guiada, que no conoce la muestra de desarrollo hasta que `DataStep` particiona— reagrupa esa columna **una sola vez** con el menor corte que deja dos niveles debajo, emite `categoria_rara_intento_reagrupar` **antes** de reintentar y `categoria_rara_reagrupada` sólo tras un ajuste exitoso; si el segundo tampoco resuelve, levanta el `BinningFitError` de siempre. **D-RAR-2**: el corte efectivo es **resultado** —va a la card de `binning` y el anexo lo distingue del declarado; el `config_hash` no se mueve y la reproducibilidad la da que la regla sea determinista—, el resumen habla sólo en el camino exitoso y en el fallido habla el **mensaje del error**, con salidas **ejecutables para una categórica** (`set_bins` escribe `user_splits`, que el motor rechaza ahí). Cambia el manejo de errores de SDD-06 §8; ningún `config_hash` se mueve y ninguna corrida que hoy termina cambia un número. Tres pasadas de Codex, las dos primeras tumbando una premisa de arquitectura cada una | [`_ENMIENDA-CATEGORIA-RARA-SIN-CLASE.md`](_ENMIENDA-CATEGORIA-RARA-SIN-CLASE.md) |
| D-FAL-1 · D-FAL-2 · D-EXC-1 | **Aprobada por Cami el 2026-09-23**, interactivamente: regla **conservadora** y `exclude()` en toda la corrida; **implementada el mismo día** (siete precisiones del código y tres pasadas de Codex en su §7; el SBA crudo termina `done`). Nace del cuarto dataset real —una muestra pública de préstamos 7(a) de la SBA con fecha, para ejercitar Desarrollo, Holdout, OOT y TTD—: ocho de 49.999 préstamos no declaran la antigüedad de la empresa y en desarrollo su bin `Missing` queda 4/0; la corrida muere en «Tramos y WoE» y `exclude()` no la salva, porque sólo escribía `selection.force_exclude`. **D-FAL-1**: la unidad es el par (variable, bin); un bin `Missing` o `Special` con observaciones y una clase en cero, con WoE empírico, recibe el WoE del tramo regular de **mayor tasa de malos observada** (el de menor WoE; empate → primera fila regular), el IV de esa fila queda en 0 y tabla, transformación, puntos y bundle leen el mismo número; el bin asignado comparte los puntos de su referencia —hereda su override y rechaza uno propio—; con un valor declarado nada cambia. **D-FAL-2**: decisión `bin_sin_clase_asignado` en el trail, campo aditivo `assigned_bins` en la card y una línea por bin en el resumen. **D-EXC-1**: `exclude()` escribe `binning.exclude_columns` y retira la variable de las listas forzadas de selección y modelo; los tramos fijados de una variable excluida quedan en suspenso, declarados, y `keep()` los reactiva. Cero perillas; D-FAL sólo entra donde la corrida iba a morir. Tres pasadas de Codex sobre el documento (tope), las dos primeras tumbando una premisa cada una | [`_ENMIENDA-FALTANTES-SIN-CLASE-Y-EXCLUSION.md`](_ENMIENDA-FALTANTES-SIN-CLASE-Y-EXCLUSION.md) |
| D-TTD-1…5 | **Aprobada por Cami el 2026-09-24/25** (S22), interactivamente, e **implementada el 2026-09-25** (lo que el código precisó y la revisión adversarial del código —tres pasadas, tope—, en su §9), con la representatividad (§5.1 a) y con las categorías no vistas puntuadas como hoy y declaradas (§5.2 a). Nace de la prueba de Cami con la muestra SBA: 6.225 préstamos sin desenlace no recibían puntaje, aunque SDD-31 §8 ya prometía que «se puntúan, no se ajustan». **D-TTD-1**: se puntúan las filas `fuera_de_modelo` con `ttd=True` —con `ttd_includes_excluded=False` ninguna— con la transformación de Holdout/OOT, sin reajustar nada; la calibración aplica el estado ajustado sin pasar por el filtro de `PDCalibrator.transform`; puntuarlas nunca detiene la corrida. **D-TTD-2**: cuatro claves aditivas `out_of_model_*` (binning, model, scorecard, calibration), en `optional_requires`, en cadena y con índice común; una clave vacía no llega al informe. **D-TTD-3**: rótulo «Fuera del ajuste (TTD)» con su composición (indeterminadas, excluidas, con desenlace apartadas por la partición). **D-TTD-4**: PSI del puntaje Desarrollo vs. fuera del ajuste en `("stability", "out_of_model_psi")`, con los cortes efectivos de estabilidad y fuera del veredicto. **D-TTD-5**: las categorías no vistas en Desarrollo se cuentan por muestra en `("binning", "unseen_categories")` y se dicen como alerta; el evento `categoria_no_vista` dice el `cat_unknown` efectivo cuando está declarado. Cero perillas; ningún número existente cambia. Tres pasadas de Codex (tope), las dos primeras tumbando premisas | [`_ENMIENDA-PUNTUAR-POBLACION-TTD.md`](_ENMIENDA-PUNTUAR-POBLACION-TTD.md) |
| D-CPY-1…6 | **Aprobada por Cami el 2026-09-24/25** (S22), interactivamente, con los rangos escritos con comparadores, e **implementada el 2026-09-25** (su §11: los bordes van exactos y los rótulos por posición de fila, por la revisión del código). Ocho hallazgos de la misma prueba; seis se corrigen aquí y tres se elevan (Hosmer-Lemeshow con muestras grandes, decisiones con motivo en el YAML, categorías no vistas y predictoras derivadas de la fecha). **D-CPY-1**: «Fuera del ajuste» para `fuera_de_modelo`, sin borrar desenlaces conocidos, también en `data.partitions` del informe. **D-CPY-2**: la línea de validación nombra la variable del CSI y el eje temporal. **D-CPY-3**: tramos legibles con dos requisitos aditivos del motor —`point_overrides` casa la etiqueta del motor o el rótulo legible y un override sin casar se declara; bordes efectivos en `("binning", "bin_edges")`—. **D-CPY-4**: p-valores «< 0,001». **D-CPY-5**: coma decimal en la prosa del sitio, con extractor de texto visible. **D-CPY-6**: Hosmer-Lemeshow dice la brecha media agregada, sin cambiar el veredicto. Dos pasadas de Codex; la reestructuración de §3 abre la implementación con su propia pasada | [`_ENMIENDA-COPY-PRUEBA-REAL-SBA.md`](_ENMIENDA-COPY-PRUEBA-REAL-SBA.md) |
...
| D-SC-1…16 | **Aprobada por Cami el 2026-09-09**, en bloque y con la recomendación de cada punto de su §8: bandas del PSI **«Estable · Revisar · Redesarrollar · No evaluable»**; estado técnico de la validación **«Pasa · Revisar · Falla»**; `validation` en «Validar un modelo existente» **se mide en la capa 2**; `eda` declara sus tres métricas D-GOB-4 y excluye del perfil por defecto las columnas del target, ambas en la capa 3 (lo segundo es minor con nota); el informe sin gobernanza **no** gana la frase nueva; **inferencia** del eje de EDA (no se enmienda D-OBL); el recálculo del PSI queda **oculto** con tarea aparte; el mínimo por grupo de Hosmer-Lemeshow **no** se cambia aquí. ⚠️ **Excepción del §8-7**: las 49 descripciones de copy público quedan como **borrador aprobado para implementar** y Cami las revisa **capa por capa contra la pantalla** (la 3 para las 17 de EDA, la 2 para las 32 de `validation`); cambiar un texto ahí es copy, no reabre el diseño. Se implementa **una capa por sesión**, 1 → 2 → 3 → 4 → 5, con la 5 pegada a la release (Cami autorizó el 2026-09-09 **encadenar** la capa siguiente en la misma sesión cuando la anterior haya cerrado entera). ✅ **Capa 1 implementada en S8 (2026-09-10)**: panel de «Selección de variables» en Resultados, resumen del peor PSI con identidad y banda de la misma observación, y una sola fuente para las cuatro palabras de banda, con espejo gateado en el front y las guías publicando las palabras. ✅ **Capa 2 implementada en S8 (2026-09-10)**: `validation` entra al formulario y a los dos trabajos del scorecard con el override del contraste por grado, ocho campos pasan a ocultos por D-SUB con su razón medida, las cuatro columnas del archivo ganan `column_role` con `columnas_inactivas()` en las tres configs de la sección, el serializer publica la card y sus cuatro tablas, y Resultados pinta «Validación formal» con la cobertura por grado. Las tres palabras del estado técnico tienen una sola fuente (`nikodym.validation.results`). **§8-4 medido y resuelto**: `check_pipeline` **sí** resuelve la familia de calibración en «Validar un modelo existente», pero la puerta de artefactos entrega el archivo sin renombrar y las tres columnas que la calibración lee están ocultas por D-SUB, así que ese trabajo **no** recibe la sección y la guía lo declara «por código». ✅ **Capa 3 implementada en S9 (2026-09-11)**: dos reglas nuevas del motor, las dos con tests nacidos rojos —con `axis="cohort"` la señal temporal es `no_evaluable` con causa (`StabilityResult.not_evaluable_reason`, tres causas, «causa ⇔ indicador no finito»), y sin fecha y con partición por cohorte el eje se **infiere** a esa cohorte con la decisión `eje_eda_inferido` en el trail—; `eda` entra al formulario como 17.ª sección («Análisis exploratorio», 2.ª del sidebar) y a los dos trabajos del scorecard **sembrada encendida**; `univariate.columns` se pinta con la casilla «Todas las columnas» (widget `multiselect_o_todas`: `null` = todas, `[]` = ninguna); las columnas del eje llevan `column_role` con `DefaultRateConfig.columnas_inactivas()`; la card gana `axis`, `axis_inferred` y `stability_not_evaluable_reason`; el serializer publica `eda` (card + tres tablas, `NaN` → `null`); Resultados pinta «Análisis exploratorio» primero entre los analíticos, con la figura decidida por el eje **efectivo**; la prosa del informe lee los mismos campos y la subsección trae sus tablas en el cuerpo (sus claves no casaban); `eda` declara sus tres métricas D-GOB-4 (respuesta 6) y excluye del perfil por defecto las columnas de las reglas del target (respuesta 8, minor con nota). Copy de §3.6 implementado, presentado a Cami contra la UI viva y **delegado** (`privado/copy-eda-capa3.md`). Los `config_hash` de los presets **no** se movieron en la capa 3. ✅ **1.13.0 publicada el 2026-09-12** por orden de Cami, con las capas 1–3 y sin las 4/5 (§0.4 de la enmienda). ✅ **2026-09-12, OK de Cami en chat a cinco mejoras** («doy el OK a todas las recomendaciones»): (a) la tabla de calidad de `eda` excluye las cuatro columnas que produce el motor (target derivado, estado, partición, TTD) —decisión que §0.3 dejaba pendiente—; (b) **capa 5, flip**: F1 y F5 encienden `eda` (eje de cohorte explícito sobre `cohorte`, perfiles sobre las variables del binning, `required_sections` gana `eda`): F1 `ec10eb43…` → `1063d6cf…`, F5 `b36318b5…` → `a7476bf2…`, F3 y F4 intactos, goldens re-anclados con la nota escrita; F5 sigue fuera de la demo; (c) ✅ **capa 4** (D-SC-13…16) implementada en la misma sesión: `ChapterSpec.requires_governance`, capítulo `model_card` tras `introduction`, `ReportInputBundle.governance` (aditivo) llenado desde `config.governance`, prosa desde lo declarado con los títulos de `GovernanceConfig` y los rótulos de `nikodym.governance.labels`, remisión en «Limitaciones y supuestos», golden sin gobernanza intacto; (d) `purpose` de la demo (D-GOB-9); (e) ensayo end-to-end con datos propios. ✅ **1.14.0 publicada el 2026-09-12** con las capas 1–5 (tag `v1.14.0` sobre `5ed1a64`). ✅ **Cierre de los abiertos por delegación de Cami, el 2026-09-12** («no son vitales, decide tú»), decididos por S9 y registrados aquí como definitivos: **(1) el eje de cohorte casi único** —se acota la RESPUESTA, no el eje: serializer, panel e informe publican como máximo **1.000** períodos/cohortes, en orden, con `total_periods` y `truncated`, y la tabla completa queda como artefacto de la corrida; §4 «una fila por período/cohorte» pasa a «hasta el tope publicado»; el motor no rechaza el eje (medido: 1.000.000 de cohortes = 73 s / 135 MB / 634 MB sin tope); se programa en la sesión siguiente con test nacido rojo—; **(2) las declaraciones de gobernanza NO admiten Markdown activo**: en QMD son texto literal de pandoc (line block, toda la puntuación escapada) como en HTML y Word; contrato definitivo, no una medida provisional; **(3) el propósito de la demo pública** queda con el texto que S9 redactó y la 1.14.0 recapturó (`scripts/capture_demo_fixtures_f1.py`, `DEMO_GOVERNANCE`); **(4) F5 no entra a la demo**: la demo publica F1 (scorecard) y F4 (IFRS 9) y no se amplía sin una razón de producto nueva; la capa 5 queda cerrada sin residual. Con esto SCORECARD-COMPLETO está **cerrado** (16/16 + los cuatro cierres) | [`_ENMIENDA-SCORECARD-COMPLETO.md`](_ENMIENDA-SCORECARD-COMPLETO.md) |
| D-PAR-1…9 | **Aprobada por Cami el 2026-09-09**, en bloque y con la recomendación de cada punto: objetivo, criterio de seis superficies y orden A → B → C; el extra `[ui]` compone también `ml`, `tuning` y `explain`, con los backends GBDT como extras que el abanico declara (opción **a**); el bloque B entra como **trabajo nuevo** «Scorecard con modelo retador»; el bloque C **se compromete como objetivo, con condiciones de entrada y sin fecha**; el gate de D-PAR-6 entra con el bloque A. 🔴 **Aprobar el objetivo NO autoriza a programar ningún bloque**: D-PAR-3 exige la enmienda medida de cada uno antes de tocar código | [`_ENMIENDA-PARIDAD-1-1.md`](_ENMIENDA-PARIDAD-1-1.md) |
| D-VAL-13…18 | **Aprobada por Cami el 2026-09-14**, interactivamente y con la recomendación de cada uno de los nueve puntos de su §8 —cerrar y retirar `FALTA-DATO-VAL-1/3`; retirar `FALTA-DATO-VAL-2` **sin sucesor** con los cortes del semáforo persistidos en fila, card, trail y prosa (la decisión contractual: el corte es una convención estadística con default, como `alpha`); copy revisado contra la pantalla; puerta por grupo de Hosmer-Lemeshow con `statistic` nulo y causa publicada; recálculo del PSI cableado por el patrón D-REQ; capas A → B → C y minor 1.16.0 con su propio OK; cotejo registrado en SDD-22 §12; cuarta palabra del estado técnico **«No evaluable»**—; **capa A implementada** (S13, 2026-09-14: D-VAL-13/14/15/18 —los tres códigos retirados del motor, la prosa, el catálogo, el copy y los tests; `GradeBinomialRecord` y la tabla `calibration` con `green_alpha`/`red_alpha`, `metric_sections.validation.traffic_light_cuts`, el trail `calibration_semaforo` con los dos cortes como umbral, la frase de los cortes en el informe y en el panel, el filtro de columnas de auditoría del renderer por clave de tabla, y el cotejo copiado a SDD-22 §12—), **capa B implementada** (S14, 2026-09-14: D-VAL-17 —`min_rows_per_group` protege cada grupo de Hosmer-Lemeshow; un HL sin veredicto publica `statistic` nulo y una de cuatro causas cerradas en la fila, en `metric_sections.validation.not_evaluable_partitions` y en la regla `calibration_hl_not_evaluable` del trail; `n_tests`/`n_failed` cuentan sólo decisiones evaluables en las cuatro familias; el estado consolidado gana `not_evaluable` («No evaluable», misma palabra que las bandas del PSI) cuando no hay evidencia evaluable alguna; el informe enumera las particiones sin veredicto en prosa y no pinta la columna de la causa; el panel la traduce junto a la fila—), **capa C implementada** (S15, 2026-09-15: D-VAL-16 —`nikodym.stability.step` expone `assemble_stability_frame` y `compute_stability` y `StabilityStep.execute` los usa (un solo camino); `StabilityConfig.requisitos_de_recalculo_declarados()` con la constante `METODO_REQUISITOS_RECALCULO`; tercer campo aditivo `ContextoDeResolucion.requisitos_de_recalculo` llenado por `Study._contexto_de_resolucion` sobre las secciones declaradas (ausente / `None` / tupla); `ValidationStep.from_config_with_context` con los tres estados y `receta_minima_de_recalculo()` (`temporal_axis="none"`, `csi_source="score_points"`, dirección del score de la ficha); `ConfigError` en el preflight ante una sección `stability` declarada e incoaccionable; `("scorecard","card")` en `optional_requires` de toda la ruta de recálculo y la guarda de dirección con `campo_de_card`; `execute` re-deriva y exige lo efectivo antes de calcular; la tabla con `source="recomputed"`, la card con `stability_source` y `stability_recompute` (DTO cerrado, reconciliados con la tabla), el trail con `stability_source` y `source` en `stability_psi`; `consume_stability` como `checkbox` con el copy de §3.2-5; `STABILITY_SOURCE_LABELS` con espejo en el front; la prosa y el panel dicen la procedencia y la receta; la guía §4 explica los dos caminos; `stability_recomputed` retirado—). Propuesta en S12 (2026-09-13); cotejo doble y trazado contra el BCE (instrucciones de febrero de 2019 y sus plantillas de reporte), Basilea 1996 y BCBS WP14: el Jeffreys y el t-test del motor **coinciden** con el BCE (`FALTA-DATO-VAL-1/3` se proponen cerradas y retiradas); el semáforo **no tiene anclaje** regulatorio y la frase «convención de Basilea (1996)» de la prosa es falsa (`FALTA-DATO-VAL-2` se propone retirada sin sucesor, con los cortes publicados en el informe); cableado real del fallback de D-VAL-2 (`consume_stability`, con `requires` derivados de la `StabilityConfig`); `min_rows_per_group` por grupo de Hosmer-Lemeshow con `statistic` nulo y causa publicada. Revisión adversarial de Codex: ocho pasadas, veintidós hallazgos, veintiuno absorbidos en el propio documento y uno **contractual** elevado en su §8-3 (¿decide un color un default del motor sin marca declarada?); además, una validación sin ninguna prueba evaluable deja de decir «Pasa» (cuarta palabra «No evaluable», elevada en §8-9). Las respuestas literales viven en el `HANDOFF` privado («Respuestas de Cami» → S12) | [`_ENMIENDA-VALIDACION-COTEJADA.md`](_ENMIENDA-VALIDACION-COTEJADA.md) |
...
3. **El informe no lleva la ficha del modelo — DIFERIDO por Cami el 2026-09-03** (§8.3 de la
   ...
   **propuesto** ese día en
   ...
   ficha; medido que el `ModelCard` se construye **después** de `report`). ➡️ **APROBADO por Cami
   el 2026-09-09** junto con el resto de D-SC; entra en la **capa 4** de esa enmienda. El abierto
   se cierra cuando esa capa esté implementada y gateada, no ahora.
...
| resumen PSI y fronteras de estabilidad | A1/B1 en [`_ENMIENDA-RESUMEN-PSI.md`](_ENMIENDA-RESUMEN-PSI.md): peor PSI score/PD con identidad y banda coherentes; `<stable`, `[stable, review)`, `≥review`. **Copy del resumen, decidido por Cami el 2026-09-03**: el rótulo «Peor PSI entre score y PD» se mantiene; los nombres de las tres bandas pasan a copy público en español (las palabras exactas se proponen y aprueban al implementar; hasta entonces `stable`/`review`/`redevelop` siguen en el motor). Cierra la única elección de producto que AGENTS.md citaba como abierta; queda pendiente de implementación. **Palabras APROBADAS por Cami el 2026-09-09**: **«Estable» · «Revisar» · «Redesarrollar» · «No evaluable»** ([`_ENMIENDA-SCORECARD-COMPLETO.md`](_ENMIENDA-SCORECARD-COMPLETO.md) D-SC-11), con **una sola fuente en Python y espejo gateado en el front** —medido que hoy conviven dos vocabularios (interfaz y prosa del informe) y los slugs en inglés en la guía; la capa 1 los unifica—. **Con esto se cierra la única elección de producto que `AGENTS.md` citaba como abierta**. ✅ **Implementado en S8 (2026-09-10, capa 1)**: `nikodym.stability.results.BAND_LABELS` es la fuente, `report/prose.py` la consume y retiró `_STABILITY_BANDS`, `chart-theme.ts` la espeja con gate en los dos sentidos, `report/renderer.py::_band_class` —tercer consumidor, que mapea por rótulo y no por slug— reconoce las palabras nuevas, y las guías publican las palabras con los identificadores sólo en la referencia de la API. El resumen A1 se pinta además en Resultados con su rótulo (D-SC-12) |
