...
| D-VAL-13…18 | **Propuesta (S12, 2026-09-13), pendiente de Cami**; cotejo doble y trazado contra el BCE (instrucciones de febrero de 2019 y sus plantillas de reporte), Basilea 1996 y BCBS WP14: el Jeffreys y el t-test del motor **coinciden** con el BCE (`FALTA-DATO-VAL-1/3` se proponen cerradas y retiradas); el semáforo **no tiene anclaje** regulatorio y la frase «convención de Basilea (1996)» de la prosa es falsa (`FALTA-DATO-VAL-2` se propone retirada sin sucesor, con los cortes publicados en el informe); cableado real del fallback de D-VAL-2 (`consume_stability`); `min_rows_per_group` por grupo de Hosmer-Lemeshow. Sin código hasta el OK: ocho decisiones en su §8; las respuestas viven en el `HANDOFF` privado | [`_ENMIENDA-VALIDACION-COTEJADA.md`](_ENMIENDA-VALIDACION-COTEJADA.md) |
