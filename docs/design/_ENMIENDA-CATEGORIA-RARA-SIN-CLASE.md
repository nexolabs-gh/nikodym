...
| 3 | **alto**: si el reajuste no alcanzaba el estado óptimo que exige el config pero su tabla salía sin bin degenerado, la regla declaraba «reagrupada» y publicaba el corte, y después la recolección descartaba la variable: el trail, la card y el resumen afirmaban una reagrupación para una variable sin WoE publicado, y se perdía una variable, cosa que §1 reserva a la persona | un reajuste no óptimo es un **reintento fallido**: el motor se detiene con su diagnóstico —«el nuevo ajuste no alcanzó una solución óptima»— y el trail sólo registra el intento. Un control negativo más |

**Tope alcanzado.** Las tres pasadas trajeron hallazgos reales y la tercera todavía uno alto, pero todos en el mismo borde —qué pasa con el estado del solver y con lo que el trail afirma—, cerrados con test y control negativo. Una de las premisas de la pasada 2 (la truncación) se midió falsa y se registró como tal. No se lanza una cuarta: el criterio declarado es el tope. Controles negativos de la capa: 8 del §4 y del resumen, más 3, 2 y 1 de las pasadas, **14 en total**.
