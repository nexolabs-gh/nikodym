...
| §4 dice que la tabla de la tasa viaja «una fila por período/cohorte» | una cohorte puede ser **cualquier** valor de la columna del usuario, incluido un `pd.Timestamp` si particionó por fecha, y el guard de finitud del serializer rechaza lo que no es JSON: la corrida terminaba bien y `/api/results` moría. Y el front fundía la cohorte numérica `2024` con la textual `"2024"` que el motor mantiene separadas. Los dos, hallazgos de la revisión adversarial de S9, reproducidos | el período o cohorte viaja nativo si lo es y como texto si no; el panel desambigua etiquetas repetidas con el tipo y usa una clave por posición |
