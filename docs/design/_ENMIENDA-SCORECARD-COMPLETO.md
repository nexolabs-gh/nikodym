...
| D-SC-12 lee el resumen de las tres claves agregadas «que ya viajan» | `psi_metric_by_comparison` es **opcional**: las cards 1.x anteriores a la enmienda del resumen PSI publicaban el máximo entre score y PD con la banda **del score**, y `ui/runs.load_results` sirve el JSON persistido sin migrarlo. Con score 0,05 y PD 0,12 esa card dice «0,1200» y «estable» a la vez (`stability/results.py` se salta ahí su propia validación de coherencia, y lo documenta) | el resumen **no se pinta** para una comparación sin identidad: la sección lo declara en una línea y conserva las series, que sí son correctas. Hallazgo de la revisión adversarial de S8, verificado contra el motor; dos gates nuevos y su control negativo. El informe no comparte la exposición: su prosa se redacta sobre el bundle de la corrida en curso, que siempre trae la identidad |
| §3 D-SC-10 dice «los umbrales activos con su rótulo del formulario» | `_thresholds_from_config` publica la **acción** siempre y anula sólo su umbral: con la estabilidad o la correlación apagadas la card sigue trayendo `stability.action` y `correlation.clustering_method`, que el motor no consume (`_apply_stability_action` sale por tabla vacía; el clustering vive dentro de `if self.correlation_enabled`); ídem `max_iv_action` sin `max_iv` | «activo» se decide por el umbral del que cada control depende, no por su propia presencia. `correlation.method` queda fuera de esa regla: la matriz se calcula aunque no se pode, y es la que produce la peor correlación de cada fila. Segundo hallazgo de la revisión adversarial de S8; el gate ata la tabla del front al serializer del motor en sus dos estados |
...
### 0.2 Lo que la implementación de la capa 2 (S8) midió distinto de este texto

Medido el 2026-09-10 al implementar D-SC-6…D-SC-9. Ninguna diferencia cambia una decisión
aprobada; se dejan escritas para que el texto no siga afirmando lo que el árbol desmiente.

| Este documento decía | Lo medido | Qué se hizo |
|---|---|---|
| §8-4 deja abierto si `validation` entra a «Validar un modelo existente», con el criterio «entra sólo si el esqueleto resulta ejecutable» | `check_pipeline` **sí** resuelve: con la PD por la puerta de artefactos deja `['data','performance','stability','validation','report']` ejecutable (medido en cinco configuraciones). El bloqueo es otro: `_materializar_externos` entrega el archivo del usuario **sin renombrar**, y las tres columnas que la calibración lee son justo las que D-SC-7 oculta por D-SUB. Ese trabajo moriría siempre con «El frame de calibración requiere la columna 'pd_calibrated'» | **no entra**, con la medición escrita en el catálogo y la guía declarando la ruta por código. El criterio de §8-4 se refina: el DAG es necesario y no suficiente |
| §6 capa 2 pide `test_jobs_ejecutables` «con el override» y aclara que ese gate sólo resuelve el DAG | correcto, y por eso el override no lo puede comprobar ningún gate de config: `_requires_for` de `validation` no cambia con `binomial_by_grade` | gate nuevo de **ejecución real** (`test_trabajo_scorecard_llega_a_done.py`): el esqueleto corre por `run_pipeline` sobre `consumo_comportamiento` hasta `done`, y un test permanente exige que **sin** el override la corrida muera pidiendo la columna de grado |
| §3.7 propone descripciones y no toca los `title` | el `title` de `min_rows_per_group` decía «Mínimo técnico por grupo HL/grado» y **contradice** lo que §0-21 midió: el mínimo NO protege cada grupo de Hosmer-Lemeshow | el título pasa a «Mínimo de operaciones para evaluar». Los demás títulos quedan como estaban y entran en la revisión de copy de Cami |
| §3.7 fija `ui_group` sin tocar el orden | `ui_order` ordena la **subsección entera**, no cada `ui_group`: los grupos «Semáforo» y «Columnas» no producen encabezado propio, así que la columna de grado aparecía entre el interruptor de Hosmer-Lemeshow y su nº de grupos. Medido en la UI viva | renumeración de `ui_order` en calibración y backtesting a una sola secuencia de lectura |
| D-SC-9 describe la tabla de estabilidad del panel como «el PSI con su banda» | `stability_metrics` trae **cuatro** métricas —`score_psi`, `pd_psi`, `csi` y `temporal_score`— y nueve de sus trece filas son CSI por variable. Sin el nombre de la variable, esas nueve se ven idénticas; y `PSI_METRIC_LABELS`, que sólo cubre las dos del resumen A1, dejaba `csi` crudo en pantalla | la tabla gana la columna «Variable» y nace `STABILITY_METRIC_LABELS` (las cuatro), con su espejo y su gate. No sustituye a `PSI_METRIC_LABELS`: responden preguntas distintas |
| D-SC-9 no dice nada del orden de las filas | el frame de calibración trae las particiones en orden de **aparición**, que depende de la partición del usuario: la misma corrida mostraba «Desarrollo, Holdout, OOT» en discriminación y «Desarrollo, OOT, Holdout» en calibración | el panel ordena las filas de calibración por el orden canónico de particiones. Es presentación pura: no filtra ni recalcula |
| D-SC-9 sólo manda sustituir `_VALIDATION_STATUS_BANDS` | `validation_intro` publicaba además el **slug** del motor en una frase del informe: «el estado técnico agregado publicado por el motor es "pass"» | la frase consume la fuente única y publica la palabra |
| §3.2 D-SC-6 dice que la opción de backtesting «se ofrece con D-EXI» | de los tres parámetros del backtesting **ninguno** es `exige_otro_campo`: su columna ya viene con nombre por defecto y ningún validador levanta por elegirlos —lo que puede faltar es la columna en el archivo, y de eso avisan el preflight y el aviso declarado—. Sí lo es la familia `backtesting` de `families`, que `_check_validation` rechaza sin `enabled` | los tres van como disponibles con el requisito dicho en su `help`; la familia va condicionada, citando el validador |
| §4 estima el formulario en «527 + 27 (validation)» a confirmar | **confirmado**: 527 → 554, con baseline por `git archive HEAD` y **0 desapariciones / 27 apariciones**, todas bajo `validation.` | los dos goldens se mueven con la enumeración escrita al lado |

### 0.3 Lo que la implementación de la capa 3 (S9) midió distinto de este texto

Medido el 2026-09-11 al implementar D-SC-1…D-SC-5. Ninguna diferencia cambia una decisión
aprobada; se dejan escritas para que el texto no siga afirmando lo que el árbol desmiente.

| Este documento decía | Lo medido | Qué se hizo |
|---|---|---|
| D-SC-4 y §6 prometen que el informe del trabajo «emite “Población y calidad de datos” con sus tablas» | `KEY_TABLES["eda"]` pedía `eda.default_rate` y `eda.quality`, y el builder publica las tablas con el nombre del frame dentro del DTO: `eda.default_rate.by_period` y `eda.quality.by_column`. Ninguna casaba, así que la subsección salía **sin tablas** en el cuerpo y las dos iban al anexo con su clave cruda por título; los perfiles por variable (`eda.univariate.profiles.<col>`) tampoco tenían título. Medido sobre el informe de la corrida real del esqueleto | las claves del contrato de capítulos pasan a las reales, con título propio; los perfiles ganan título dinámico como las tablas de binning. Gate de ejecución real sobre el HTML |
| D-SC-2 fija el `valor` de la decisión `no_evaluable` sólo para el eje de cohorte («eje de cohorte sin cronología») | la decisión de «< 2 períodos» ya escribía `"<2 períodos"` y un test la anclaba; la tercera causa no tenía decisión | las tres causas escriben su frase en `valor` (`<2 períodos` se conserva); `StabilityResult` lleva el identificador. La regla «causa ⇔ indicador no finito» se prueba en los dos sentidos |
| §3.1 D-SC-1 dice que el widget anulable «se crea como `ui_widget` `multiselect_o_todas`» sin decir qué hace el toggle genérico | el formulario ya pinta todo `X \| None` con un interruptor «Activar» que, apagado, dice «(desactivado)»: sobre `columns` habría rotulado «desactivado» a lo que significa «todas» | el widget nuevo **esquiva** ese toggle y antepone la casilla «Todas las columnas» al multiselect; las dos columnas del eje siguen con el toggle genérico (abierto de copy compartido, `privado/copy-eda-capa3.md`) |
| §4 dice «`serialize_study`: claves nuevas `eda` y `validation`» sin más | `dump_dto` **falla** ante un `NaN`, y la card de `eda` lo trae en `stability_value` cada vez que la señal no se evalúa —el caso del esqueleto del scorecard— | la card de `eda` se serializa con una coacción acotada a sus **dos** ausencias declaradas (`overall_default_rate`, `stability_value`; `Inf` y cualquier otro `NaN` siguen fallando, y un valor ausente sin causa es contrato roto); las demás cards siguen por `dump_dto`. La primera versión coaccionaba todas las cards y la revisión adversarial de S9 lo acusó. Gate con la corrida real |
| §3.5 y §8-6 declaran las tres métricas D-GOB-4 de `eda` como «tres escalares con productor» | `stability_flagged` es un `bool`, y el canal de métricas **rechaza** un `bool` como contrato roto (D-GOB-2) | viaja como `1.0`/`0.0`, dicho en el productor |
| §6 capa 3 lista los goldens del formulario «554 → ~571» a confirmar | **confirmado**: 554 → 571 y 437 → 454, con baseline por `git archive HEAD`: **0 desapariciones / 17 apariciones**, todas bajo `eda.` | los dos goldens se mueven con la enumeración escrita al lado |
| §6 capa 3 cuenta cinco censos que se mueven al añadir una sección | son **seis**: los tres espejos escritos a mano de `CONFIG_SECTIONS` (copy, defaults efectivos, decisiones) no se comparaban con el front, así que una sección nueva podía quedar **sin gate de copy** en silencio | gate nuevo que compara el espejo con `schema.ts`, en orden, y los otros dos con él |
| §3.6 (nota al pie) y §1 dan por hecho que la calidad por columna describe «variables» | `DataQualityProfiler` describe **todas** las columnas del frame, incluidas las que produce el motor (`target`, `label_status`, `partition`, `ttd`): sobre desarrollo, partición y TTD salen «casi constante» por construcción | se deja como está (comportamiento del motor desde su primera versión) y se declara en el CHANGELOG como sabido; excluirlas es decisión pendiente |
| §5 dice que sin fecha ni cohorte «el preflight lo anticipa cuando el esquema declarado no tiene ninguna columna de fecha» | el preflight recibe **nombres** de columna, no tipos —un Parquet trae fechas nativas que ningún esquema declara— y la partición por cohorte vive en otra sección que `ContextoConfig` no transporta: afirmarlo desde los nombres sería el falso positivo que D-INV-4 prohíbe. Y el gate D-INV-7 exigía que `eda` declarara su política de invariantes (censo que §6 no contaba) | `DefaultRateConfig.requisitos_incumplidos()` declara la única invariante afirmable —eje de cohorte sin su columna— y deja escrito por qué la simétrica no; anticiparla exige que el perfil del dataset diga qué columnas son de fecha y que el contexto diga si la partición es por cohorte: extensión medida y **pendiente** |
| §4 dice que la tabla de la tasa viaja «una fila por período/cohorte» | una cohorte puede ser **cualquier** valor de la columna del usuario, incluido un `pd.Timestamp` si particionó por fecha, y el guard de finitud del serializer rechaza lo que no es JSON: la corrida terminaba bien y `/api/results` moría. Y el front fundía la cohorte numérica `2024` con la textual `"2024"` que el motor mantiene separadas; desambiguar por el tipo JSON no bastaba, porque JSON también funde `2024` con `2024.0` y el motor no (segunda pasada). Hallazgos de la revisión adversarial de S9, reproducidos | el período o cohorte viaja nativo si lo es y como texto si no, y la fila gana `period_type`: el tipo con que el motor lo distinguió (`int`, `float`, `bool`, `str`, `datetime`, `period`), que el serializer añade y el tipo TS espeja con gate; el panel desambigua etiquetas repetidas con ese tipo y usa una clave por posición y tipo |
| §0-3 y D-SC-4 dan por hecho que el informe emite la subsección «con sus tablas y figuras» (medido en F5: «figuras SVG») | lo medido en F5 eran las **recetas** de `eda.figures` en el bundle, no figuras dibujadas: el renderer nunca tuvo constructor de gráficos para `eda`, así que la prosa anunciaba «N figuras» que ningún documento reproducía (segunda pasada de la revisión adversarial de S9) | la tasa en el tiempo (barras por cohorte, línea por fecha) y los perfiles por tramo (hasta 12 paneles, y el título dice cuántas variables quedaron fuera) salen por el pipeline de gráficos del informe —SVG en HTML/QMD, PNG en Word— y la prosa dice lo que el documento reproduce; el Anexo B deja de emitir el slot vacío rotulado con la clave interna `eda.figures` (mueve el golden del renderer en esas tres líneas, medidas; el golden del informe de `test_report_step` queda intacto). El productor conserva la receta de un punto con un solo período, como D-SC-5 (b) manda; es el constructor el que la omite, y no cuenta como degradación |
| D-SC-5 dice «línea si `axis="period"`, barras si `axis="cohort"`» y §5 sólo prevé el período con `n_eligible == 0` como «`default_rate = NaN`, marcado» en la tabla | dibujar esa ausencia como barra de altura cero la hacía pasar por un 0 % real, y una línea que puentea el hueco dibuja una tasa que el motor no calculó; y el eje de cohorte acepta **cualquier** columna, así que una casi única materializaba una barra y un rótulo por fila (5.000 cohortes: 27 s y un SVG desbordado, medido). Tercera pasada de la revisión adversarial de S9 | la ausencia se marca (cruz en la base y leyenda) y nunca se dibuja como cero, en la tasa y en los perfiles; la línea se corta en el hueco; las cohortes se acotan a `_MAX_EDA_RATE_BARS = 60` en el informe y `EDA_MAX_RATE_BARS` en el panel, con gate que iguala los dos, y el título o la nota lo dicen; los rótulos del eje se ralean a 24. Siete tests nacidos rojos; 5.000 cohortes pasan a 0,23 s |
| §4 dice que el período o cohorte viaja «nativo si lo es» | un entero mayor que `2**53 - 1` no es nativo para el navegador: `JSON.parse` funde `9007199254740992` y `9007199254740993` en el mismo número (cuarta pasada de la revisión adversarial de S9, reproducida en vitest) | el entero fuera del rango seguro de JavaScript viaja como texto y `period_type` sigue diciendo `int`; test nacido rojo |
| §4 dice que `default_rate.by_period` viaja «una fila por período/cohorte», sin tope | el eje de cohorte acepta cualquier columna: una casi única produce una fila por operación y la respuesta se construye entera —medido: 1.000.000 de cohortes son 73 s, 135 MB de JSON y 634 MB de pico al serializar—. Acotar la respuesta cambia la forma pública del payload y rechazar ese eje es una regla nueva del motor: las dos son enmienda, no fix (cuarta pasada) | **no se toca**; queda como decisión de Cami en el HANDOFF, con tres opciones y recomendación. Las figuras y el panel ya acotan lo que dibujan |
| §3.6 propone descripciones y no toca los `title` | trece rótulos seguían en jerga del motor («eje period», «univariado», «pre-binning») | reescritos en idioma de negocio con el criterio de la capa 2 y presentados a Cami contra la UI viva; delegó (`privado/copy-eda-capa3.md`) |

### 0.4 La release 1.13.0 (S9) se cortó antes de las capas 4 y 5

Por orden de Cami del 2026-09-11 —una demo con datos propios el lunes 14—, la 1.13.0 se publica
