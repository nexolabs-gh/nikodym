...
|---|---|---|
| `provisioning.internal` | **neutro** | Provisión por exposición y tasa de pérdida sobre grupos homogéneos —descompuesta en PD · LGD o provista directamente; los grupos los formas tú o los deriva el motor por banda—. No conoce ninguna cartera normativa ni ninguna tabla de supervisor. **Su estado de fábrica también es neutro**: la columna de cartera se llama `portfolio` por defecto y ninguna ayuda de su formulario cita una norma. Antes no era así —el default se llamaba `cmf_portfolio`, de modo que un banco de otro país tenía que renombrar su columna para correr un cálculo que no conoce ninguna norma— y es lo que se corrigió. |
