...
4. Re-verifica el **artefacto ya escrito** (3 secciones no nulas, capítulo del informe con la cifra
   del sobrecosto, que el informe ya no declare las *provisiones* como fase posterior —G5— y que no
...
def verify_business(results: dict[str, Any]) -> dict[str, float]:
    ...
    """
    _verificar_lineage(results, "results.json")
...
def verify_artifacts() -> None:
    ...
    - El informe no publica el caveat de working tree sucio.
    """
    ...
    assert _CAVEAT_GIT_SUCIO not in html_raw, (
        f"report.html publica el caveat «{_CAVEAT_GIT_SUCIO}»: el informe de la demo declara él "
