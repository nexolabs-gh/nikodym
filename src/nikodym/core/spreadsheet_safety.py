...
etiqueta la tasa— y ninguno los produce el motor: por eso se neutralizan al EXPORTAR, no al
calcular, y la mitigación es la que recomienda OWASP: anteponer una comilla simple, que fuerza la
celda a texto y deja el valor legible tal cual. Sólo se tocan las celdas de texto que empiezan por
uno de esos prefijos; los números, los lógicos, las fechas y el resto del texto viajan intactos.
Hallazgo de la revisión adversarial de la serie de mantenimiento posterior a la 1.14.0.
"""
...
def neutralize_formula_prefixes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    Recorre las columnas de texto (``object``/``string``) y el índice si también lo es; anteponer
    una comilla simple es lo único que cambia. Devuelve el mismo objeto cuando no hay nada que
    proteger —así un export sin celdas activas sigue siendo byte a byte el de siempre— y nunca
    muta el frame recibido.
    """
    ...
    for column in frame.columns:
        ...
        if not _is_text_like(serie):
            continue
        protegida = serie.map(_neutralize_value)
        if not protegida.equals(serie):
            if result is frame:
    ...
    if _is_text_like(frame.index):
        ...
        if not indice.equals(frame.index):
            if result is frame:
                result = frame.copy(deep=True)
            result.index = indice
    return result
...
def _is_text_like(values: Any) -> bool:
    """``object`` o ``string``: sólo ahí puede vivir un texto que una planilla lea como fórmula.
    ...
    if dtype is None:
        return False
    return bool(pd.api.types.is_object_dtype(dtype)) or isinstance(dtype, pd.StringDtype)
