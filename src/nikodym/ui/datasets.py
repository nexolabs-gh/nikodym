...
__all__ = ["list_datasets", "materialize"]
...
def materialize(dataset_id: str, *, workdir: Path) -> Path:
    ...
    ----------
    dataset_id : str
        Identificador del dataset (clave del registro). Uno desconocido levanta ``UiDatasetError``.
    workdir : Path
    ...
    ------
    UiDatasetError
        Si el ``dataset_id`` es desconocido o la ruta escaparía del ``workdir`` (*path traversal*).
    """
